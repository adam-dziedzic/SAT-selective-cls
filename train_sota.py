...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        trainset = dataset(root='~/datasets/SVHN', split='train', download=True,
                           transform=transform_train)
        testset = dataset(root='~/datasets/SVHN', split='test', download=True,
                          transform=transform_test)
        ...
        trainset = dataset_utils.CatsDogs(root='~/datasets/cats_dogs',
                                          split='train',
                                          transform=transform_train, resize=64)
        testset = dataset_utils.CatsDogs(root='~/datasets/cats_dogs',
    ...
    model = models.__dict__[args.arch](
        num_classes=num_classes if args.loss == 'ce' else num_classes + 1,
    ...
    optimizer = optim.SGD(model.parameters(), lr=state['lr'],
                          momentum=args.momentum,
                          weight_decay=args.weight_decay)

    title = args.dataset + '-' + args.arch + ' o={:.2f}'.format(reward)
    logger = Logger(
        os.path.join(save_path, 'eval.txt' if args.evaluate else 'log.txt'),
        title=title)
    logger.set_names(
        ['Epoch', 'Learning Rate', 'Train Loss', 'Test Loss', 'Train Err.',
         'Test Err.'])

    # if only for evaluation, the training part will not be executed
    if args.evaluate:
        print('\nEvaluation only')
        assert os.path.isfile(resume_path), 'no model exists at "{}"'.format(
            resume_path)
        checkpoint = torch.load(resume_path)
        model.load_state_dict(checkpoint['state_dict'])
        if use_cuda: model = model.cuda()
        test(testloader, model, criterion, args.epochs, use_cuda,
             evaluation=True)
        return
    ...
    for epoch in range(0, args.epochs):
        ...
        filepath = os.path.join(
            save_path,
            "{}-seed-{:d}-epoch-{:d}".format(
                args.loss, args.manualSeed, epoch + 1) + ".pth")
        model_state = {
            'epoch': epoch + 1,
            'arch': args.arch,
        ...
        #     save_path,
        #     "{}-seed-{:d}-epoch-{:d}".format(args.loss, args.manualSeed, epoch) + ".pth")
        # if os.path.isfile(last_path): os.remove(last_path)
        # append logger file
        logger.append(
            [epoch + 1, state['lr'], train_loss, test_loss, 100 - train_acc,
             100 - test_acc])
...
def test(testloader, model, criterion, epoch, use_cuda, evaluation=False):
    ...
    for batch_idx, batch_data in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            if epoch >= args.pretrain:
                ...
                abstention_results.extend(zip(list(reservation.numpy()), list(
                    predictions.eq(targets.data).numpy())))
            else:
    ...
    if epoch >= args.pretrain:
        ...
        for coverage in expected_coverage:
            print('{:.0f}: {:.3f}, '.format(coverage, sum(
                sorted_correct[int(size / 100 * coverage):])), end='')
        print('')
    return (losses.avg, top1.avg)
...
def evaluate(testloader, model, use_cuda):
    ...
    abortion, correct = torch.stack(abortion_results[0]), torch.stack(
        abortion_results[1])
...
def bisection_method(abortion, correct, results):
    ...
    while True:
        ...
        if passed_up / len(correct) * 100. < expected_coverage[0]:
            upper *= 2.
        else:
            break
    test_thres = 1.
...
def _get_num_covered_and_confident_error_idxs(desired_coverages, preds,
                                              confidences, y_true):
...
def eval_converage(logits, confidences, labels,
    ...
    assert len(logits) == (
            np.sum(correct, axis=0) + len(confident_error_idxs[-1]))
...
if __name__ == '__main__':
    ...
    for i in range(len(reward_list)):
        ...
        save_path = base_path + 'o{:.2f}'.format(reward)
        if args.evaluate:
            resume_path = os.path.join(save_path,
                                       '{:d}.pth'.format(args.epochs))
